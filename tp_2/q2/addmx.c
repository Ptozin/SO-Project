...
int main(int argc, char *argv[]) {
    ...
    if(ptr == MAP_FAILED){
        fprintf(stderr, "Mapping Failed. Check if the first line of the file is of type \"n\"x\"m\" ");
        fclose(f1);
        fclose(f2);
        return EXIT_FAILURE;
    }
    ...
    for(int i = 0; i < line1; i++){
        for(int j = 0; j < column1; j++){
            if(!fscanf(f1, "%d", &ptr[i*column1+j])){
                fprintf(stderr, "Error while loading matrix1. Check if number of lines and columns correspond with the first line.\n");
                fclose(f1);
                fclose(f2);
                return EXIT_FAILURE;
            }
            if(i == line1-1 && j == column1-1) {
                if(!feof(f1)) {
                    fprintf(stderr, "Error while loading matrix1. Make sure there are no newlines at the end of the file.\nCheck if number of lines and columns correspond with the first line.\n");
                    fclose(f1);
                    fclose(f2);
                    return EXIT_FAILURE;
                }
    ...
    for(int i = 0; i < line1; i++){
        for(int j = 0; j < column1; j++){
            if(!fscanf(f2, "%d", &ptr[line1*column1 + i*column2+j])){
                fprintf(stderr, "Error while loading matrix2. Check if number of lines and columns correspond with the first line.\n");
                fclose(f1);
                fclose(f2);
                return EXIT_FAILURE;
            }
            if(i == line1-1 && j == column1-1) {
                if(!feof(f2)) {
                    fprintf(stderr, "Error while loading matrix2. Make sure there are no newlines at the end of the file.\nCheck if number of lines and columns correspond with the first line.\n");
                    fclose(f1);
                    fclose(f2);
                    return EXIT_FAILURE;
                }
