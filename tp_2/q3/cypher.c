    ...
    for (i = 0; s[i] != '\0'; i++) {
        for(k = 0; k < LINE_SIZE && k < keywords_size; k++) {
            if (strstr(&s[i], keywords[k][0]) == &s[i]) {
                aux1 = s[i + strlen(keywords[k][0])];
                aux2 = s[i-1];
                if(((aux1 >= 65 && aux1 <= 90) || (aux1 >= 97 && aux1 <= 122)) || ((aux2 >= 65 && aux2 <= 90) || (aux2 >= 97 && aux2 <= 122))) {
                    ;
                }
                else {
            ...
            if (strstr(&s[i], keywords[k][1]) == &s[i]) {
                aux1 = s[i + strlen(keywords[k][1])];
                aux2 = s[i-1];
                if(((aux1 >= 65 && aux1 <= 90) || (aux1 >= 97 && aux1 <= 122)) || ((aux2 >= 65 && aux2 <= 90) || (aux2 >= 97 && aux2 <= 122))) {
                    continue;
                }
...
int main(int argc, char* argv[]) {
    ...
    if(argc != 1) {
        fprintf(stderr, "usage: cypher < file\n");
        return EXIT_FAILURE;
    }
